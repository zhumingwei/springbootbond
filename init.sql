...
CREATE TABLE account (
  `id` bigint(32) not null AUTO_INCREMENT COMMENT '主账户ID',
#   `username` varchar(20) NOT NULL COMMENT '用户名',
  `password` varchar(32) not NULL COMMENT '密码',
  `phonenumber` varchar(15) unicode NULL COMMENT '手机号',

  `uid` bigint(20) NOT NULL COMMENT '用户id外键连接用户表id',
  `createby` bigint(20) NOT NULL DEFAULT 0 COMMENT '用户表id',
  `createdate` datetime NOT NULL DEFAULT NOW() COMMENT '创建时间',
  `updateby` bigint(20) NOT NULL DEFAULT 0 COMMENT '用户id',
...
CREATE TABLE user(
  `id` bigint(32) NOT NULL AUTO_INCREMENT PRIMARY KEY COMMENT '用户id' ,
